...
function App() {
  ...
  // שים לב: בדוקר זה localhost, ב-Vite רגיל זה 127.0.0.1
  // אם יש בעיה תשנה חזרה ל-http://127.0.0.1:8000/tasks/
  ...
  const toggleTask = async (id, currentStatus, title) => {
    try {
      // עדכון אופטימיסטי (UI מתעדכן לפני השרת)
      ...
      fetchTasks() // במקרה של שגיאה, רענן את המצב האמיתי
    }
  }
  ...
  const handleAiBreakdown = async (id) => {
    setAiLoadingId(id)
    try {
      const response = await axios.post(`${API_URL}${id}/ai-breakdown`)
      const newSubtasks = response.data
      setTasks(prevTasks => [...newSubtasks.reverse(), ...prevTasks])
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 flex items-start justify-center py-12 px-4 font-sans text-slate-800">
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2 font-medium">Powered by FastAPI, React & AI</p>
        </div>
            ...
            {loading ? (
              ...
              <ul className="space-y-3">
                {tasks.map((task) => (
                  <li 
                    key={task.id} 
                    className={`group flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-200 ${task.is_completed ? 'bg-slate-50' : 'hover:-translate-y-0.5'}`}
                  >
                    {/* צד שמאל: צ'קבוקס וטקסט */}
                    ...
                    <div className="flex items-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      ...
                      {!task.is_completed && (
                        <button 
                          onClick={(e) => { e.stopPropagation(); handleAiBreakdown(task.id); }}
                          disabled={aiLoadingId === task.id}
                          className="flex items-center gap-1 px-3 py-1.5 bg-purple-50 text-purple-600 text-sm font-semibold rounded-lg hover:bg-purple-100 transition-colors disabled:opacity-50"
                        >
                          {aiLoadingId === task.id ? (
